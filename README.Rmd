...
In the case of the weibull psychometric function (`fit_weibull()`), two additional parameters are typically specified _a priori_: the y-intercept (i.e., the expected probability when `x` is zero), which could be
...
In the case of the logistic psychometric function (`fit_logistic()`), the values of the function as `x` becomes arbitrarily small or arbitrarily large are often specified _a priori_ and fixed to be the same. These parameters
